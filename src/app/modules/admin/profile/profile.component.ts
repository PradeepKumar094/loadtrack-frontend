...
export class ProfileComponent implements OnInit {
  ...
  // Toggle panels — clicking same button closes it
  openEdit() {
  ...
  onPhotoSelected(event: Event) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      this.profileErrorMsg = 'Image must be less than 2MB';
      return;
    }
  ...
  saveProfile() {
    ...
    this.http.put<ApiResponse<UserProfile>>(this.url, {
      ...
      error: (err) => {
        this.savingProfile = false;
        this.profileErrorMsg = err.error?.message || 'Update failed';
      }
    });
  }
  ...
  changePassword() {
    if (this.newPassword !== this.confirmPassword) {
      this.pwdErrorMsg = 'New password and confirm password do not match';
      return;
    }
    if (this.newPassword.length < 6) {
      this.pwdErrorMsg = 'Password must be at least 6 characters';
      return;
    }
    this.savingPwd = true;
    this.pwdErrorMsg = '';
    this.http.put<ApiResponse<void>>(`${this.url}/change-password`, {
      ...
      next: () => {
        ...
        this.pwdSuccessMsg = 'Password changed! Logging out...';
        this.activePanel = null;
        setTimeout(() => { this.authService.logout(); }, 2000);
      },
      error: (err) => {
        this.savingPwd = false;
        this.pwdErrorMsg = err.error?.message || 'Password change failed';
      }
    });
  }
